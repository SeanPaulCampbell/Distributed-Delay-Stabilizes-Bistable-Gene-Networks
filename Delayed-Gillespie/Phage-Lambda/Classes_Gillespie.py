...
class Reaction:
    propensities_list = ['mobius_propensity', 'decreasing_hill_propensity',
                         ...
                         'binomial_propensity', 'product_propensity']
    ...
    def distribution(self, markov=-1):
        return getattr(self, Reaction.distributions_list[self.dist_id])(markov)
    ...
    def binomial_propensity(self, state_vector):
        x = state_vector[self.parts_of_vec[0]]
        value = 1
        for index in range(self.prop_par[0]):
            value = value * (x - index) / (index + 1)
        return self.prop_par[1] * value

    def product_propensity(self, state_vector):
        product = 1
        for index in self.parts_of_vec:
            product = product * state_vector[index]
        return product * self.prop_par[0]

    ''' Distributions start here.
    After adding a distribution to the list of definitions,
    append the name to distr_list. '''

    def gamma_distribution(self, markov):
        if markov == -1:
            if self.dist_par[1] == 0:
                return self.trivial_distribution(markov)
            mean = self.dist_par[0]
            stdev = self.dist_par[1]
        else:
            if self.dist_par[markov][1] == 0:
                return self.trivial_distribution(markov)
            mean = self.dist_par[markov][0]
            stdev = self.dist_par[markov][1]
        return random.gamma(shape=(mean / stdev) ** 2, scale=stdev ** 2 / mean)

    def trivial_distribution(self, markov):
        if markov == -1:
            mean = self.dist_par[0]
        else:
            mean = self.dist_par[markov][0]
    ...
    def bernoulli_distribution(self, markov):
        if markov == -1:
            mean = self.dist_par[0]
            stdev = self.dist_par[1]
        else:
    ...
    def uniform_distribution(self, markov):
        if markov == -1:
            mean = self.dist_par[0]
            stdev = self.dist_par[1]
        else:
