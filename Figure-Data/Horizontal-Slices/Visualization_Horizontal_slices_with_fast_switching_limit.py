...
''' Getting distributed data '''
folder = "2025-8-21/"
system = "CR_bernoulli"

bernoulli_residency_counts = np.load(folder+system+"_residency_counts.npy")
bernoulli_mean_residencies = np.load(folder+system+"_mean_residencies.npy")
bernoulli_cv_residencies = np.load(folder+system+"_cv_residencies.npy")
bernoulli_attempt_counts = np.load(folder+system+"_attempt_counts.npy")
bernoulli_mean_attempt_times = np.load(folder+system+"_thresh_means.npy")
bernoulli_cv_attempt_times = np.load(folder+system+"_thresh_cvs.npy")
bernoulli_mean_number_of_attempts = np.load(folder+system+"_geometric_means.npy")
bernoulli_cv_number_of_attempts = np.load(folder+system+"_geometric_cvs.npy")

bernoulli_sterr_residencies = np.divide(mean_residencies * cv_residencies, np.sqrt(residency_counts))
bernoulli_sterr_number_of_attempts = np.divide(mean_number_of_attempts * cv_number_of_attempts, np.sqrt(residency_counts))
bernoulli_sterr_attempt_times = np.divide(mean_attempt_times * cv_attempt_times, np.sqrt(attempt_counts))
...
bernoulli_mean_residencies = bernoulli_mean_residencies[1,:-5]
bernoulli_mean_attempt_times = bernoulli_mean_attempt_times[1,:-5]
bernoulli_mean_number_of_attempts = bernoulli_mean_number_of_attempts[1,:-5]
bernoulli_sterr_residencies = bernoulli_sterr_residencies[1,:-5]
bernoulli_sterr_number_of_attempts = bernoulli_sterr_number_of_attempts[1,:-5]
bernoulli_sterr_attempt_times = bernoulli_sterr_attempt_times[1,:-5]

residency0 = list(mean_residencies[:,0])
attempt_time0 = list(mean_attempt_times[:,0])
attempt_num0 = list(mean_number_of_attempts[:,0])
bernoulli_residency0 = bernoulli_mean_residencies[0]
bernoulli_attempt_time0 = bernoulli_mean_attempt_times[0]
bernoulli_attempt_num0 = bernoulli_mean_number_of_attempts[0]
...
bernoulli_mean_residencies = bernoulli_mean_residencies / bernoulli_residency0
bernoulli_mean_attempt_times = bernoulli_mean_attempt_times / bernoulli_attempt_time0
bernoulli_mean_number_of_attempts = bernoulli_mean_number_of_attempts / bernoulli_attempt_num0
bernoulli_sterr_residencies = bernoulli_sterr_residencies / bernoulli_residency0
bernoulli_sterr_attempt_times = bernoulli_sterr_attempt_times / bernoulli_attempt_time0
bernoulli_sterr_number_of_attempts = bernoulli_sterr_number_of_attempts / bernoulli_attempt_num0
...
dark = "darkorchid"
light = "orchid"
axes[0].errorbar(cv_range, bernoulli_mean_residencies, yerr=bernoulli_sterr_residencies, fmt='o', capsize=3, ms=2, ecolor=light, mfc=dark, mec=dark)
axes[1].errorbar(cv_range, bernoulli_mean_number_of_attempts, yerr=bernoulli_sterr_number_of_attempts, fmt='o', capsize=3, ms=2, ecolor=light, mfc=dark, mec=dark)
axes[2].errorbar(cv_range, bernoulli_mean_attempt_times, yerr=bernoulli_sterr_attempt_times, fmt='o', capsize=3, ms=2, ecolor=light, mfc=dark, mec=dark)
...
figure.legend([r"$r=2^0$",r"$r=2^4$",r"$r=2^8$",r"$r=\infty$"])
plt.tight_layout()
plt.show()
