...
for index in range(mus):
    plt.errorbar(cv_range, mean_residencies[index], yerr=sterr_residencies[index], fmt='o', capsize=3, ms=1, ecolor=ecolors[index], mfc=colors[index], mec=colors[index])
plt.title(r"Corepressive Toggle Bernoulli Distributed")
plt.xlabel(r"$\cv[\tau]$")
...
figure.suptitle(r"Corepressive Toggle Bernoulli Distributed")
