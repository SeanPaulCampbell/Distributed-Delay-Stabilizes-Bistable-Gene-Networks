...
system = "CR3S_thresh"
path_to_storage = "2025-4-25/distributed/" ### edit to the correct path!
...
framed_data = pd.concat([pd.DataFrame([{r"$\cv[\tau]$" : cv_range[index],
                                  "time to hit separatrix" : total_residencies[0][index][jndex]} for jndex in range(len(total_residencies[0][index]))])
                                for index in range(cvs)])
print(framed_data)

ax = sns.histplot(framed_data,
                  x="time to hit separatrix",
                  hue=r"$\cv[\tau]$", palette="icefire", common_norm=False, log_scale=True, kde=True,
                  stat="density", element="step", alpha=0.2)
plt.title(r"histograms of transition attempt times for Bernoulli distribution vs $\cv[\tau]$")
